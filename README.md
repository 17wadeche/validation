...
Draft responses carry a `placeholders` map and `answers` list so you can paste values directly—only placeholder text should change while headings, bullets, tables, and formatting remain intact. The agent fills every placeholder it can using the provided template, examples, and code before requesting anything else; if details are still missing, it includes concise follow-up questions alongside the answers instead of withholding them.
