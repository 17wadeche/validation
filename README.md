...
Draft responses are expected to mirror the template exactly—only placeholder text should change while headings, bullets, tables, and formatting remain intact. The agent fills every placeholder it can using the provided template, examples, and code before requesting anything else; if details are still missing, it includes concise follow-up questions alongside the draft instead of withholding it.
