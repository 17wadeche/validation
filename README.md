...
1. Prepare inputs:
   ...
   - Paths to the program code to be reflected in the documentation. Passing a folder now builds a prioritized snapshot (README, requirements/metadata, configs, and source files) plus a short manifest so the agent sees the most important parts of any tech stack while keeping noise out. Aim the path at the project root for the best coverage.
...
3. Open `http://localhost:8000` and upload your template, examples, and code context, then click **Generate draft**. The agent now runs the planning step automatically behind the scenes (inventorying placeholders, asking clarifying questions, and mapping where to write) before drafting, so you only press one button. The modernized layout highlights each step in cards with clear labels, so you can see what is saved and what will be reused at a glance. The UI uses files only for templates and examples—guidance should be embedded as blue placeholder text inside the template itself. **Generate draft with MedtronicGPT**, **Remember credentials on this machine**, and **Remember template and examples on this machine** are checked by default; provide your `subscription-key`, `api-token`, `refresh-token`, and desired `model` (use any gateway model ID—your entry is saved for next time; API version `3.0`, base URL `https://api.gpt.medtronic.com` or `https://api.gpt-dev.medtronic.com` via the Base URL picker). The UI sends `subscription-key`, `api-token`, `refresh-token`, and `api-version` as headers and defaults to the path template `/models/{model}`—aligning with the working Postman example. Adjust the path if your gateway expects an alternate route.
   ...
   - Pick the release type (Initial vs Update). Initial release is the default; when you select Update, extra upload slots appear for **Previous** and **Current** code/context. Files and folders added there are tagged as OLD/NEW in the prompt so MedtronicGPT knows which content changed. Code/context uploads now list every selected file (including folder contents) so you can deselect anything noisy before submitting.
...
- The page also runs a coverage check against the uploaded template: any detected placeholders that remain empty are listed, and if credentials are provided the app automatically resends the gaps plus context for another pass before routing the document. When the model must guess, it returns the value with `confidence: "low"` so you can confirm or replace it.
  - When the model returns follow-up questions, the UI automatically sends them back with the full context for a best-effort answer before showing them, tagging guesses with `confidence: "low"` so you know what to confirm. Questions that are just lists of `<placeholder>` tokens are hidden and treated as missing placeholders to auto-fill in the background.
